<template>
  <div v-if="showWidget && isPublicRoute" class="pointer-events-none fixed bottom-0 right-0 z-[60] flex flex-col items-end gap-4 p-4 sm:p-6 md:p-8">
    ...
    >
      <div
        v-show="open"
        class="pointer-events-auto relative flex min-h-0 max-h-[92vh] max-w-[min(100vw-1rem,75rem)] flex-col overflow-hidden rounded-xl border-2 border-white/15 bg-zinc-900 shadow-2xl shadow-black/50 ring-2 ring-white/15"
        role="dialog"
        :aria-label="$t('common.supportChat.title')"
        :style="{ width: `${panelWidth}px`, height: `${panelHeight}px` }"
      >
        <div
          class="absolute left-0 top-0 z-20 flex h-10 w-10 cursor-nwse-resize select-none items-start justify-start rounded-br-lg border border-white/10 bg-zinc-800/90 text-gray-400 hover:border-primary-400/40 hover:bg-violet-950/10 hover:text-primary-400"
        ...
        <div class="flex items-center justify-between border-b border-white/10 px-5 py-4 pl-12">
          <div class="flex items-center gap-3 text-white">
            <ChatBubbleLeftRightIcon class="h-6 w-6 shrink-0 text-white" aria-hidden="true" />
            <span class="text-base font-semibold tracking-tight sm:text-lg">{{ $t('common.supportChat.title') }}</span>
          </div>
          <button
            type="button"
            class="rounded-lg p-1.5 text-gray-400 hover:border hover:border-primary-400/50 hover:bg-violet-950/10 hover:text-white"
        ...
        <div class="border-b border-white/10 px-5 py-3">
          <div class="flex flex-wrap gap-2 text-sm">
            <RouterLink
              ...
              class="rounded-md border border-white/20 px-3 py-1.5 text-gray-300 hover:border-primary-400 hover:bg-violet-950/10 hover:text-white"
              @click="open = false"
            >
          ...
          >
            <div class="flex flex-wrap gap-2">
              <button
                ...
                type="button"
                :disabled="loading"
                class="rounded-md border border-white/20 bg-white/5 px-3 py-2 text-left text-sm text-gray-200 hover:border-primary-400 hover:bg-primary-100/10 hover:text-white disabled:pointer-events-none disabled:opacity-50"
        ...
        <form class="border-t border-white/10 p-4 sm:p-5" @submit.prevent="send">
          <div class="flex gap-3">
            ...
            <input
              ...
              class="min-w-0 flex-1 rounded-lg border border-white/20 bg-white/5 px-4 py-3 text-base text-white placeholder-gray-400 focus:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-400 disabled:opacity-50"
            />
            <button
              type="submit"
              class="flex-shrink-0 rounded-lg bg-white px-4 py-3 text-black hover:bg-primary-100 disabled:opacity-50"
    ...
    <button
      type="button"
      class="pointer-events-auto flex h-[4.25rem] w-[4.25rem] sm:h-[4.75rem] sm:w-[4.75rem] items-center justify-center rounded-full border-2 border-black/50 bg-white text-black shadow-2xl shadow-black/40 ring-4 ring-black/25 hover:bg-primary-100 hover:ring-black/40 focus:outline-none focus:ring-4 focus:ring-primary-400"
...
function formatInline(s) {
  ...
  t = t.replace(/`([^`]+)`/g, (_, inner) => {
    ...
    codes.push(
      '<code class="rounded bg-violet-950/50 px-1.5 py-0.5 font-mono text-[0.9em] text-white">' +
        inner +
        '</code>'
    )
    return `§§C${idx}§§`
...
function formatMarkdownBlocks(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      blocks.push(
        '<ol class="my-2 list-decimal space-y-2 pl-5 marker:font-medium marker:text-white/90">' +
          items.join('') +
          '</ol>'
      )
      continue
    ...
    if (/^[-*]\s+/.test(trimmed)) {
      ...
      blocks.push(
        '<ul class="my-2 list-disc space-y-2 pl-5 marker:text-white">' + items.join('') + '</ul>'
      )
      continue
    }
