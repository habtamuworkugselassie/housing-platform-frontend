  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-white">Building Management</h1>
          <p class="mt-2 text-sm text-gray-400">{{ $t('admin.manageBuildings') }}</p>
        </div>
        <button
          @click="showCreateModal = true"
          class="px-4 py-2 bg-white text-black rounded-md hover:bg-primary-100 font-medium transition-colors"
        >
          + Add Building
        </button>
      </div>
