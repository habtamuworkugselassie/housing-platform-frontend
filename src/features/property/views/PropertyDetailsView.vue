  ...
  <div class="min-h-screen bg-black text-white">
    ...
    <div v-else class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8">
      ...
      <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
        ...
        <div class="p-4 sm:p-6 border-b border-gray-200">
          <div class="flex flex-col gap-3">
            <div class="flex items-start gap-3">
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 flex-1">{{ property.title }}</h1>
              <span
                :class="{
                  'bg-green-100 text-green-800 border-green-200': property.status === 'AVAILABLE',
                  'bg-yellow-100 text-yellow-800 border-yellow-200': property.status === 'RESERVED',
                  'bg-gray-100 text-gray-800 border-gray-200': property.status === 'SOLD'
                }"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div class="flex-1">
                ...
                <div class="flex flex-col gap-0.5">
                  <div v-if="property.priceETB" class="text-2xl sm:text-3xl font-bold text-primary-600">
                    {{ formatPrice(property.priceETB, 'ETB') }}
                    <span v-if="property.category === 'FOR_RENTAL'" class="text-base text-gray-600 font-normal">/month</span>
                  </div>
                  <div v-if="property.priceUSD" class="text-lg sm:text-xl font-semibold text-gray-600">
              ...
              <div class="flex gap-2">
                <button 
                  ...
                  class="px-4 py-2 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 transition-colors flex items-center justify-center gap-2 text-sm"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button 
                  ...
                  :class="[
                    ...
                    isFavorite ? 'border-red-300 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                  ]"
                >
                  <svg 
                    :class="['w-4 h-4', isFavorite ? 'text-red-600 fill-current' : 'text-gray-700']" 
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
                ...
                <button 
                  ...
                  class="px-3 py-2 border-2 border-gray-300 rounded-lg hover:border-gray-400 transition-colors"
                >
        ...
        <!-- Quick Stats -->
        <div class="p-3 sm:p-4 bg-gradient-to-r from-primary-50 to-blue-50">
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            <div v-if="property.bedrooms" class="text-center">
              <div class="flex items-center justify-center w-8 h-8 bg-white rounded-full mx-auto mb-1 shadow-sm">
                <svg class="w-4 h-4 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                </svg>
              </div>
              <p class="text-xl font-bold text-gray-900">{{ property.bedrooms }}</p>
              <p class="text-xs text-gray-600">{{ $t('property.bedrooms') }}</p>
            </div>
            <div v-if="property.bathrooms" class="text-center">
              ...
              <p class="text-xs text-gray-600">{{ $t('property.bathrooms') }}</p>
            </div>
            <div v-if="property.area" class="text-center">
              ...
              <p class="text-xs text-gray-600">{{ $t('property.sqMeters') }}</p>
            </div>
            <div v-if="property.floorNumber" class="text-center">
              ...
              <p class="text-xs text-gray-600">{{ $t('property.floor') }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <dl class="space-y-3">
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.type') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ property.type || $t('property.na') }}</dd>
                  </div>
                  <div v-if="property.constructionStatus" class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.construction') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ property.constructionStatus }}</dd>
                  </div>
                  <div v-if="property.constructionPercentage !== null && property.constructionPercentage !== undefined" class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.progress') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ property.constructionPercentage }}%</dd>
                  </div>
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.category') }}</dt>
                    <dd>
                      <span :class="{
                  ...
                  <div v-if="property.isFullyFurnished" class="flex items-center justify-between py-2">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.furnished') }}</dt>
                    <dd>
                      <span class="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-semibold">{{ $t('common.fullyFurnished') }}</span>
                    </dd>
                  </div>
                </dl>
              </div>
              ...
              <div>
                ...
                <dl class="space-y-3">
                  <div class="flex items-start justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.address') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900 text-right max-w-[60%]">{{ property.address }}</dd>
                  </div>
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.city') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ property.city }}</dd>
                  </div>
                  <div v-if="property.state" class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.state') }}</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ property.state }}</dd>
                  </div>
                  <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.country') }}</dt>
                  ...
                  <div v-if="property.zipCode" class="flex items-center justify-between py-2">
                    <dt class="text-sm font-medium text-gray-600">{{ $t('property.zipCode') }}</dt>
          ...
          <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            ...
            <div class="bg-gray-200 rounded-xl overflow-hidden" style="height: 400px; position: relative;">
              <div class="h-full w-full relative bg-gray-200">
                ...
                <div class="absolute top-4 left-4 z-10 bg-white px-4 py-2 rounded-lg shadow-md">
                  <h4 class="text-sm font-semibold text-gray-900">{{ property.city }}{{ property.country ? `, ${property.country}` : '' }}</h4>
                  <p class="text-xs text-gray-600 mt-1">{{ property.address }}</p>
                </div>

                <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">
                  <div class="bg-primary-600 text-white px-4 py-2 rounded-lg shadow-lg border-2 border-white">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div v-if="company" class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            ...
            <div class="space-y-4">
              <div>
                ...
                <p v-if="company.description" class="text-sm text-gray-600 mb-4">{{ company.description }}</p>
              </div>

              <div class="space-y-3">
                <a v-if="company.email" :href="`mailto:${company.email}`" class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">
                ...
                <a v-if="company.phoneNumber" :href="`tel:${company.phoneNumber}`" class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">
                  <div class="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center group-hover:bg-primary-200 transition-colors">
                    <svg class="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                  </div>
                  <span class="text-sm font-medium text-gray-900 flex-1">{{ company.phoneNumber }}</span>
                </a>

                <div v-if="company.address" class="p-3 bg-gray-50 rounded-lg">
                  <div class="flex items-start gap-3">
                    ...
                    <p class="text-sm text-gray-700">{{ company.address }}{{ company.city ? `, ${company.city}` : '' }}{{ company.country ? `, ${company.country}` : '' }}</p>
                  </div>
                </div>

                <a v-if="company.website" :href="company.website" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">
                  <div class="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center group-hover:bg-primary-200 transition-colors">
                    <svg class="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/>
                    </svg>
                  </div>
                  <span class="text-sm font-medium text-primary-600 flex-1 truncate">{{ company.website }}</span>
                </a>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="property.realEstateCompanyName" class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $t('property.realEstateCompany') }}</h3>
            <p class="text-lg font-semibold text-gray-900 mb-2">{{ property.realEstateCompanyName }}</p>
            <p class="text-sm text-gray-500">{{ $t('property.contactInformationNotAvailable') }}</p>
          </div>

          <!-- Gallery Section -->
          <div v-if="property.images && property.images.length > 0" class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div
                ...
                class="relative aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer group hover:opacity-90 transition-opacity"
              >
                <img
                  :src="image.imageUrl"
        ...
        <div v-if="financingOffers.length" class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
          <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ $t('property.financingOptions') }}</h2>
          <div class="space-y-4">
            <div
              v-for="offer in financingOffers"
              :key="offer.id"
              class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-xl p-6 border border-blue-200 hover:shadow-md transition-shadow"
            >
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                  ...
                  <p v-if="offer.creditProductDescription" class="text-sm text-gray-700 mb-4">{{ offer.creditProductDescription }}</p>
                  
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div v-if="offer.interestRate || offer.specialInterestRate" class="bg-white rounded-lg p-4 text-center">
                      <p class="text-xs font-medium text-gray-500 mb-1">{{ $t('loan.interestRate') }}</p>
                      <p class="text-2xl font-bold text-primary-600">{{ (offer.specialInterestRate || offer.interestRate) }}%</p>
                    </div>
                    <div v-if="offer.maxLoanAmount" class="bg-white rounded-lg p-4 text-center">
                      <p class="text-xs font-medium text-gray-500 mb-1">{{ $t('loan.maxLoanAmount') }}</p>
                      <p class="text-lg font-bold text-gray-900">{{ formatPrice(offer.maxLoanAmount, offer.currency || 'ETB') }}</p>
                    </div>
                    <div v-if="offer.maxTenureMonths" class="bg-white rounded-lg p-4 text-center">
                      <p class="text-xs font-medium text-gray-500 mb-1">{{ $t('loan.maxTenure') }}</p>
                      <p class="text-lg font-bold text-gray-900">{{ offer.maxTenureMonths }} {{ $t('loan.tenureMonths') }}</p>
                    </div>
                    <div v-if="offer.maxLoanToValueRatio" class="bg-white rounded-lg p-4 text-center">
                  ...
                  <div v-if="offer.specialTerms" class="mt-4 p-4 bg-white rounded-lg border border-blue-100">
                    <p class="text-xs font-semibold text-gray-700 mb-2">{{ $t('loan.specialTerms') }}</p>
                    <p class="text-sm text-gray-700">{{ offer.specialTerms }}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 sm:p-8">
        ...
        <div v-if="company">
          <div class="mb-6">
            ...
            <p v-if="company.description" class="text-sm text-gray-600">{{ company.description }}</p>
          </div>
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="flex-1">
                ...
                <p class="text-base font-semibold text-gray-900">{{ company.phoneNumber }}</p>
              </div>
            </a>
