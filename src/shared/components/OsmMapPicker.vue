  ...
  <div class="osm-map-picker rounded-xl overflow-hidden border border-white/10 bg-zinc-900 flex flex-col" :style="{ height }">
    ...
    <div v-if="searchable" class="flex gap-2 p-2 border-b border-white/10 shrink-0">
      <input
        ...
        class="flex-1 min-w-0 border border-white/20 bg-white/5 text-white placeholder-gray-400 rounded-lg py-2 px-3 text-sm focus:ring-2 focus:ring-primary-400 focus:border-primary-400"
        @keydown.enter.prevent="doSearch"
      />
      <button
        type="button"
        :disabled="searching"
        class="px-3 py-2 rounded-lg bg-white text-black hover:bg-primary-100 disabled:opacity-50 disabled:bg-white/50 text-sm font-medium shrink-0"
        @click="doSearch"
      >
        {{ searching ? '…' : 'Search' }}
      </button>
